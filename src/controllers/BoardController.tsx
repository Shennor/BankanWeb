...
export const getBoardInfo = async (boardId: number) => {
    try {
        BoardInstance.defaults.headers.common["Authorization"] = `Bearer ${store.getState().token}`
...
const getLists = async (lists: IList[]) => {
    ListInstance.defaults.headers.common["Authorization"] = `Bearer ${store.getState().token}`
    const responses = [];
